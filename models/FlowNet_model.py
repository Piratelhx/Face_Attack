...
class FlowNet(nn.Module):
    ...
    def __init__(self, in_ch = 5 , flow_ch = 10):
        ...
        # flow unit
            ...
            # refine unit
            self.flow6 = flow(1024,flow_ch)
            self.flow6_up = upsample(flow_ch,flow_ch)
            self.deconv5 = leaky_deconv(1024,256)

            # in_ch = 512 + 256 + 10 = 778
            self.flow5 = flow(778, flow_ch)
            self.flow5_up = upsample( flow_ch, flow_ch)
            self.deconv4 = leaky_deconv(778, 256)

            #in_ch = 512 + 256 + 10 = 778
            self.flow4 = flow(778,flow_ch)
            self.flow4_up = upsample(flow_ch, flow_ch)
            self.deconv3 = leaky_deconv(778, 128)

            #in_ch = 256 + 128 + 10
            self.flow3 = flow(394,flow_ch)
            self.flow3_up = upsample(flow_ch, flow_ch)
            self.deconv2 = leaky_deconv(394, 64)

            #in_ch = 64 + 128 + 10 = 202
            self.flow2 = flow(202,flow_ch)
            self.flow2_up = upsample(flow_ch, flow_ch)
            self.deconv1 = leaky_deconv(202, 64)

            #in_ch = 64 + 64 + 10
            self.flow1 = flow(138,flow_ch)
            self.flow1_up = upsample(flow_ch, flow_ch)
            self.deconv0 = leaky_deconv(138, 64)

            #in_ch = 5 + 64 + 10 = 79
            self.concat0_conv1 = conv_activation(79, 16,kernel_size = 7, stride = 1, padding = 3,activation = 'selu', init_type = 'w_init')
            self.concat0_conv2 = conv_activation(16, 16,kernel_size = 7, stride = 1, padding = 3,activation = 'selu', init_type = 'w_init')
            self.flow_12 = flow(16,flow_ch)
    ...
    def forward(self, input_):
        ...
        concat0 = torch.cat((input_,deconv0,flow1_up),1)
